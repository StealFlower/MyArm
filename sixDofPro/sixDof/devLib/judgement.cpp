...
void Judgement::usart3Config(void)
{
    ...
    */
    ...
    /*配置 DMA*/
    LL_DMA_Struct.PeriphOrM2MSrcAddress = (uint32_t)&USART3->RDR;
    LL_DMA_Struct.MemoryOrM2MDstAddress = (uint32_t)judgeDataBuffer;
    ...
    LL_DMA_Struct.NbData = (uint32_t)JUDGE_BUFFER_LENGTH;
    LL_DMA_Struct.PeriphRequest = LL_DMAMUX_REQ_USART3_RX;
    LL_DMA_Struct.Priority = LL_DMA_PRIORITY_HIGH;
    LL_DMA_Init(DMA1, LL_DMA_CHANNEL_4, &LL_DMA_Struct);
...
{
	rx_len = JUDGE_BUFFER_LENGTH - LL_DMA_GetDataLength(DMA1, LL_DMA_CHANNEL_4) + judgementFullCount * JUDGE_BUFFER_LENGTH; //获取裁判当前已经接收的长度
	...
	while (rx_len > read_len + 1) //当没读满时
	{
		...
		rx_len = JUDGE_BUFFER_LENGTH - LL_DMA_GetDataLength(DMA1, LL_DMA_CHANNEL_4) + 
...
void Judgement::customSend(u8 count)
{
    ...
    sendUnionData.senderId = WHO_AM_I;
	...
	usart3SendBytes(&sendUnionData, count);
}
 ...
 * 
 */
void Judgement::customCtrlSend()
{
	customCtrlData.frameHeader.cmdid = 0x302;
	int count = sizeof(FrameHeader) + sizeof(customCtrl) + 2;
	for(u8 i=0;i<30;i++)
	{
		customCtrlData.customCtrl.data[i] = i*3+13;
		
	}
	customCtrlData.frameHeader.sof = 0xa5;
	customCtrlData.frameHeader.dataLength = count - sizeof(FrameHeader) - 2;
	Append_CRC8_Check_Sum((unsigned char *)&customCtrlData, 5);
	Append_CRC16_Check_Sum((unsigned char *)&customCtrlData, count);

	usart3SendBytes(&customCtrlData, count);
}
/**
 * @brief 发送机器人交互数据
 * 
...
{
	sendUnionData.dataCmdId = dataCmdId;//己方机器人通信 0x0200~0x02ff
	//接收者ID
...
void Judgement::graphicDraw(u8 sendGraphics)
{
	if (sendGraphics <= 1)
	{
		sendUnionData.dataCmdId = 0x101;//绘制一个图形
		sendGraphics = 1;
	}
	else if (sendGraphics <= 2)
	{
		sendUnionData.dataCmdId = 0x102;//绘制两个图形
		sendGraphics = 2;
	}
	else if (sendGraphics <= 5)
	{
		sendUnionData.dataCmdId = 0x103;//绘制五个图形
		sendGraphics = 5;
	}
	else if (sendGraphics <= 7)
	{
		sendUnionData.dataCmdId = 0x104;//绘制七个图形
		sendGraphics = 7;
	}
	sendUnionData.frameHeader.cmdid = 0x0301;
...
{
	sendUnionData.frameHeader.cmdid = 0x0301;
	sendUnionData.dataCmdId = 0x110;//绘制字符
...
/**
 * @brief 向评论区中发送文字，支持中文（24赛季加入）
 * 
 */
void Judgement::commentSend()
{
    commentData.frameHeader.cmdid = 0x0308;
    commentData.senderId = WHO_AM_I;//发送者ID
    commentData.receiverId = (WHO_AM_I == 2) ? 0x102 : 0x166;//接收者ID，默认给工程操作手选手端发送
    //commentData.receiverId = (WHO_AM_I == 2) ? 0x106 : 0x16A;//接收者ID，默认给云台手发送
    int count = sizeof(FrameHeader) + sizeof(CommentData) + 2;//

    commentData.frameHeader.sof = 0xa5;
    commentData.frameHeader.dataLength = count - sizeof(FrameHeader) - 2;
    Append_CRC8_Check_Sum((unsigned char *)&customCtrlData, 5);
	Append_CRC16_Check_Sum((unsigned char *)&customCtrlData, count);

    usart3SendBytes(&commentData, count);
}
/**
 * @brief 删除图形
 * 
...
{
	sendUnionData.frameHeader.cmdid = 0x0301;
	sendUnionData.dataCmdId = 0x100;//删除图形
...
void Judgement::mapCommandSend()
{
	...
	usart3SendBytes(&mapCommandData, count);
}
 ...
 * @param len 长度
 */
void Judgement::usart3SendBytes(void *ptr, u8 len)
{
	static LL_DMA_InitTypeDef LL_DMA_Struct = {0};
	static u8 firstLoad = 1; //首次加载时加载配置缺省值
	if (firstLoad == 1)
	{
        firstLoad = 0;
        LL_DMA_Struct.PeriphOrM2MSrcAddress = (uint32_t)&USART3->TDR;
        LL_DMA_Struct.Direction = LL_DMA_DIRECTION_MEMORY_TO_PERIPH;
        ...
        LL_DMA_Struct.PeriphRequest = LL_DMAMUX_REQ_USART3_TX;
        LL_DMA_Struct.Priority = LL_DMA_PRIORITY_HIGH;
        LL_DMA_Init(DMA1, LL_DMA_CHANNEL_5, &LL_DMA_Struct);
        LL_DMA_EnableChannel(DMA1, LL_DMA_CHANNEL_5);
        
		/* DMA1_Channel5_IRQn interrupt configuration */
        NVIC_SetPriority(DMA1_Channel5_IRQn, NVIC_EncodePriority(NVIC_GetPriorityGrouping(),3, 1));
        NVIC_EnableIRQ(DMA1_Channel5_IRQn);

        LL_DMA_DisableIT_TC(DMA1, LL_DMA_CHANNEL_5);
	}
    LL_DMA_DisableChannel(DMA1, LL_DMA_CHANNEL_5);
    LL_DMA_Struct.MemoryOrM2MDstAddress = (uint32_t)ptr;//内存基地址
    LL_DMA_ClearFlag_TC5(DMA1);
    LL_DMA_Struct.NbData = (uint32_t)len;//dma缓存大小
    LL_DMA_Init(DMA1, LL_DMA_CHANNEL_5, &LL_DMA_Struct);
    LL_DMA_EnableChannel(DMA1, LL_DMA_CHANNEL_5);
}
...
{
    return WHO_AM_I + ROBOT_CLIENT_ID_DIFF;
